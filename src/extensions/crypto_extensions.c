...
static void bn_divmod_general(uint32_t *a, uint32_t alen,
                              uint32_t *b, uint32_t blen,
...
static void bn_divmod_single(uint32_t *a, uint32_t alen,
                             uint32_t  b,
                             uint32_t *q, uint32_t *r) {
  uint64_t rem = 0;
  ...
  if (argn == 2 && lbm_is_array_r(args[0]) && lbm_is_array_r(args[1])) {
    ...
    if (bn_cmp(ap, alen, bp, blen) < 0) {
      ...
      if (lbm_heap_allocate_array(&qv,   alen * sizeof(uint32_t)) &&
        ...
        bn_divmod_general(ap, alen, bp, blen,
                          (uint32_t*)lbm_dec_array_r(qv)->data,
                          (uint32_t*)lbm_dec_array_r(rv)->data,
...
static void bn_mod(uint32_t *a,      uint32_t alen,
                   uint32_t *n,      uint32_t nlen,
                   uint32_t *result, uint32_t *rlen,
                   uint32_t *scratch_q,
                   uint32_t *scratch_an,
                   uint32_t *scratch_bn) {
  if (bn_cmp(a, alen, n, nlen) < 0) {
    memcpy(result, a, alen * sizeof(uint32_t));
    memset(result + alen, 0, (nlen - alen) * sizeof(uint32_t));
    *rlen = alen;
  } else if (nlen == 1) {
    bn_divmod_single(a, alen, n[0], scratch_q, result);
    *rlen = 1;
  } else {
    bn_divmod_general(a, alen, n, nlen, scratch_q, result, scratch_an, scratch_bn);
    *rlen = bn_normalize_len(result, nlen);
  }
}
...
static void bn_modexp(uint32_t *base, uint32_t blen,
  ...
  for (uint32_t i = bits; i > 0; i--) {
    ...
    bn_mul(result, rlen, result, rlen, tmp_mul);
    uint32_t tlen = bn_normalize_len(tmp_mul, rlen + rlen);
    bn_mod(tmp_mul, tlen, n, nlen, result, &rlen,
           scratch_q, scratch_an, scratch_bn);

    // if bit is set: result = result * base mod n
    if (bn_get_bit(exp, i - 1)) {
      bn_mul(result, rlen, base, blen, tmp_mul);
      tlen = bn_normalize_len(tmp_mul, rlen + blen);
      bn_mod(tmp_mul, tlen, n, nlen, result, &rlen,
             scratch_q, scratch_an, scratch_bn);
    }
  }
...
static lbm_value ext_bn_modexp(lbm_value *args, lbm_uint argn) {
  ...
  if (argn == 3 &&
    ...
    uint32_t blen  = bn_normalize_len(base, base_arr->size / sizeof(uint32_t));
    uint32_t elen  = bn_normalize_len(exp,  exp_arr->size  / sizeof(uint32_t));
    uint32_t nlen  = bn_normalize_len(n,    n_arr->size    / sizeof(uint32_t));
    ...
    if (lbm_heap_allocate_array(&result,    nlen         * sizeof(uint32_t)) &&
      ...
      bn_modexp(base, blen, exp, elen, n, nlen,
