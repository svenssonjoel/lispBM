...
static bool vector_float_destruct(lbm_uint value) {
  lbm_free((void*)value);
  return true;
}

static lbm_value vector_float_allocate(unsigned int size) {
  vector_float_t *mem = lbm_malloc( 1 * sizeof(unsigned int) +
                                    size * sizeof(float));
  if (!mem) return ENC_SYM_MERROR;
  mem->size = size;
  lbm_value res;
  lbm_custom_type_create((lbm_uint)mem,
                         vector_float_destruct,
                         vector_float_desc,
                         &res);
  return res;
}
...
static lbm_value ext_vector_to_list(lbm_value *args, lbm_uint argn) {
  ...
  if (argn != 1  ||
      ((lbm_uint)lbm_get_custom_descriptor(args[0]) !=
       (lbm_uint)vector_float_desc)) {
...
static lbm_value ext_saxpy(lbm_value *args, lbm_uint argn ) {
  ...
  if (((lbm_uint)lbm_get_custom_descriptor(x) == (lbm_uint)vector_float_desc) &&
      ((lbm_uint)lbm_get_custom_descriptor(y) == (lbm_uint)vector_float_desc) &&
      lbm_is_number(a)) {
