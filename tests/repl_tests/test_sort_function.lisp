...
(define sort_test1 (let ((result (sort < '(3 1 4 1 5 9 2 6))))
...
(define sort_test6 (eq (trap (sort)) '(exit-error eval_error)))
(define sort_test7 (eq (trap (sort <)) '(exit-error eval_error)))
(define sort_test8 (eq (trap (sort < '(1 2 3) extra-arg)) '(exit-error eval_error)))

; sort with invalid comparator
(define sort_test9 (eq (trap (sort "not-function" '(1 2 3))) '(exit-error type_error)))
(define sort_test10 (eq (trap (sort 42 '(1 2 3))) '(exit-error type_error)))
(define sort_test11 (eq (trap (sort 'symbol '(1 2 3))) '(exit-error type_error)))
(define sort_test12 (eq (trap (sort t '(1 2 3))) '(exit-error type_error)))
...
(define sort_test21 (let ((result (sort < '(3 1 3 1 3))))
                      (and (is-sorted t < result) (= (length result) 5))))

; sort with large list
(define sort_test22 (let ((input (range 10 1))
                          (result (sort < input)))
                      (and (is-sorted t < result) (= (length result) 10))))
