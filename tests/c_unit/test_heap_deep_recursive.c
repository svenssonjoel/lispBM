...
int test_deep_recursive_gc_stack_overflow(void) {
  ...
  // Create a structure deeper than this so that the explicit GC stack
  // is exhausted and marking falls back to pointer-reversal (DSW) traversal.
  ...
  // The explicit GC stack (size 256) is too small for this structure (depth
  // 512), so marking is expected to fall back to pointer-reversal traversal
  // partway through and complete without crashing the evaluator.
  printf("Triggering garbage collection...\n");
  lbm_request_gc(); // this is fine, GC called from evaluator thread.
  lbm_continue_eval();

  timeout = 0;
  while (lbm_get_eval_state() != EVAL_CPS_STATE_RUNNING && timeout < 5) {
    sleep_callback(1000);
    timeout++;
  }

  if (lbm_get_eval_state() == EVAL_CPS_STATE_DEAD) {
    printf("Evaluator did not survive garbage collection\n");
    return 0;
  }

  printf("Garbage collection completed without crash\n");
  printf("Post-GC stack max usage (still bounded by GC stack size): %u\n", lbm_get_gc_stack_max());

  // GC stack usage should never exceed the configured GC stack size: once
  // exhausted, marking continues via pointer-reversal instead of growing
  // the explicit stack further.
  if (lbm_get_gc_stack_max() > lbm_get_gc_stack_size()) {
    printf("GC stack max usage exceeded GC stack size\n");
    return 0;
  }
